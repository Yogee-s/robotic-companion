...
class TextToSpeech:
    ...
    def __init__(self, config: dict):
        self._default_engine = config.get("engine", "piper")
        self.voice = config.get("voice", "af_heart")
        self.speaking_rate = config.get("speaking_rate", 1.1)
        self.output_sample_rate = config.get("output_sample_rate", 22050)
        self.lang = config.get("lang", "en-us")
        self._piper_model = config.get("piper_model", "en_US-hfc_female-medium")

        project_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self._models_dir = os.path.join(project_dir, "models")
    ...
    def _load_all(self):
        """Load all available engines, then activate the default."""
        self._has_piper = self._load_piper()
        self._has_kokoro = self._load_kokoro()

        # Activate the configured default
        if self._default_engine == "kokoro" and self._has_kokoro:
    ...
    def set_engine(self, engine: str):
        ...
        if engine == "kokoro" and self._has_kokoro:
            self._activate("kokoro")
        elif engine == "piper" and self._has_piper:
            self._activate("piper")
        else:
